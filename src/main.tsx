...
import { buildDraftCount, createInventoryCount, loadInventoryState, PermissionDeniedError, saveArticle, saveClient, saveClientArticleAssignments, saveCountLine, saveWarehouse, sendInventoryCountToReview } from './supabaseInventory';
...
function App() {
  ...
  const actions = {
    ...
    saveArticle: (article: Article, message: string) => runWrite(() => saveArticle(supabase!, article), message),
    saveClient: (client: Client, message: string) => runWrite(() => saveClient(supabase!, client), message),
    saveWarehouse: (warehouse: Warehouse, message: string) => runWrite(() => saveWarehouse(supabase!, warehouse), message),
    saveAssignments: (clientId: string, articleIds: string[], activeArticleIds: string[]) => runWrite(() => saveClientArticleAssignments(supabase!, clientId, articleIds, activeArticleIds), 'Asignaciones guardadas'),
  };
  ...
  const router = () => {
    ...
    if (path === '/catalogo') return <Catalog state={state} go={go} setModal={setModal} actions={actions}/>;
    if (path === '/clientes') return state.role === 'Supervisor' ? <ClientsView state={state} go={go} back={back} update={update} setModal={setModal} actions={actions}/> : <StateCard title="Permiso denegado" message="Solo un Supervisor puede administrar clientes y bodegas."/>;
...
type ServerActions = {
  createCount: (count: InventoryCount, after: () => void) => Promise<boolean>;
  saveLine: (line: CountLine, after?: () => void) => Promise<boolean>;
  sendToReview: (count: InventoryCount, after: () => void) => Promise<boolean>;
  saveArticle: (article: Article, message: string) => Promise<boolean>;
  saveClient: (client: Client, message: string) => Promise<boolean>;
  saveWarehouse: (warehouse: Warehouse, message: string) => Promise<boolean>;
  saveAssignments: (clientId: string, articleIds: string[], activeArticleIds: string[]) => Promise<boolean>;
};
...
function Catalog({ state, go, setModal, actions }: Pick<ScreenProps,'state'|'go'|'setModal'|'actions'>) { const [query,setQuery]=useState('');const [filter,setFilter]=useState<'Activos'|'Todos'|'Inactivos'>('Activos');const list=state.articles.filter(a=>(filter==='Todos'||filter==='Activos'&&a.active||filter==='Inactivos'&&!a.active)&&(`${a.name} ${a.code} ${a.barcode ?? ''}`).toLowerCase().includes(query.toLowerCase()));const edit=(article?:Article)=>setModal(<ArticleForm state={state} article={article} close={()=>setModal(null)} actions={actions}/>);return <><Header title="Catálogo de artículos" back={()=>history.back()} badge={<span className="role-chip">{state.role}</span>}/><div className="screen-pad catalog"><label className="search-box"><Icon name="search"/><input aria-label="Buscar por nombre o código" placeholder="Buscar por nombre o código" value={query} onChange={e=>setQuery(e.target.value)}/></label><div className="catalog-tools"><select aria-label="Filtro de artículos" value={filter} onChange={e=>setFilter(e.target.value as typeof filter)}><option>Activos</option><option>Todos</option><option>Inactivos</option></select>{state.role==='Supervisor'&&<Button onClick={()=>edit()}><Icon name="plus"/>Nuevo artículo</Button>}</div>{list.length?list.map(a=><Card className="catalog-card" key={a.id}><button className="catalog-card-button" onClick={()=>state.role==='Supervisor'?edit(a):undefined}><div><h2>{a.name}</h2><p>{a.code}</p><Status status={a.active?'Activo':'Inactivo'}/></div>{state.role==='Supervisor'&&<Icon name="edit"/>}<hr/><dl><dt><Icon name="box"/>Presentación</dt><dd>{a.presentation==='Caja'?`Caja de ${a.unitsPerBox} unidades`:'Unidad'}</dd><dt><Icon name="package"/>Conversión</dt><dd>{a.presentation==='Caja'?`1 caja = ${a.unitsPerBox} unidades`:'1 unidad = 1 unidad'}</dd><dt><Icon name="calendar"/>Vencimiento</dt><dd><span className={a.appliesExpiry?'expiry-yes':'expiry-no'}>{a.appliesExpiry?'Aplica vence':'No aplica vence'}</span></dd></dl></button></Card>):<Card className="empty-card"><Icon name="search"/><p>Sin resultados para esta búsqueda.</p></Card>}</div></>; }
function ArticleForm({ state, article, close, actions }: { state:AppState;article?:Article;close:()=>void;actions?:ServerActions }) { const [name,setName]=useState(article?.name??'');const [code,setCode]=useState(article?.code??'');const [presentation,setPresentation]=useState<UnitType>(article?.presentation??'Caja');const [factor,setFactor]=useState(String(article?.unitsPerBox??24));const [expiry,setExpiry]=useState(article?.appliesExpiry??false);const [active,setActive]=useState(article?.active??true);const [error,setError]=useState('');const [saving,setSaving]=useState(false);const save=async()=>{const normalized=code.trim().toUpperCase();const factorValue=Number(factor);if(!name.trim()||!normalized||!/^\d+$/.test(factor)||factorValue<1||factorValue>9999){setError('Completa los campos obligatorios con un factor válido.');return}if(state.articles.some(a=>a.id!==article?.id&&a.code.toUpperCase()===normalized)){setError('El código ya existe.');return}setError('');setSaving(true);const next:Article={id:article?.id??id('article'),barcode:article?.barcode,name:name.trim(),code:normalized,presentation,unitsPerBox:presentation==='Unidad'?1:factorValue,appliesExpiry:expiry,active};const ok=await actions?.saveArticle(next,article?'Artículo actualizado':'Artículo creado');setSaving(false);if(ok)close();else setError('No se pudo guardar. Verifica permisos o conexión.');};return <Modal title={article?'Editar artículo':'Nuevo artículo'} onClose={close}><label>Nombre<input value={name} onChange={e=>setName(e.target.value)}/></label><label>Código<input value={code} onChange={e=>setCode(e.target.value.toUpperCase())}/></label><label>Presentación<select value={presentation} onChange={e=>setPresentation(e.target.value as UnitType)}><option>Caja</option><option>Unidad</option></select></label><label>Unidades por caja<input disabled={presentation==='Unidad'} inputMode="numeric" value={presentation==='Unidad'?'1':factor} onChange={e=>setFactor(e.target.value)}/></label><label className="check-row"><input type="checkbox" checked={expiry} onChange={e=>setExpiry(e.target.checked)}/>Aplica vencimiento</label><label className="check-row"><input type="checkbox" checked={active} onChange={e=>setActive(e.target.checked)}/>Activo</label>{error&&<p className="field-error">{error}</p>}<Button onClick={save} disabled={saving}>{saving?'Guardando...':'Guardar artículo'}</Button><Button variant="secondary" onClick={close}>Cancelar</Button></Modal> }

function ClientsView({ state, go, back, setModal, actions }: ScreenProps) { const [query,setQuery]=useState('');const [open,setOpen]=useState('client-ahorro');const clients=state.clients.filter(c=>c.name.toLowerCase().includes(query.toLowerCase()));const form=(client?:Client)=>setModal(<ClientForm state={state} client={client} close={()=>setModal(null)} actions={actions}/>);const warehouseForm=(clientId:string,warehouse?:Warehouse)=>setModal(<WarehouseForm state={state} clientId={clientId} warehouse={warehouse} close={()=>setModal(null)} actions={actions}/>);const manage=(client:Client)=>setModal(<Assignments client={client} state={state} close={()=>setModal(null)} actions={actions}/>);return <><Header title="Clientes y bodegas" back={back} badge={<span className="role-chip">{state.role}</span>}/><div className="screen-pad clients"><label className="search-box"><Icon name="search"/><input aria-label="Buscar cliente" placeholder="Buscar cliente" value={query} onChange={e=>setQuery(e.target.value)}/></label>{state.role==='Supervisor'&&<Button onClick={()=>form()}><Icon name="plus"/>Nuevo cliente</Button>}{clients.map(client=>{const isOpen=open===client.id;const wh=state.warehouses.filter(w=>w.clientId===client.id);const assigned=state.clientArticles.filter(a=>a.clientId===client.id&&a.active).length;return <Card className="client-card" key={client.id}><button className="client-head" onClick={()=>setOpen(isOpen?'':client.id)}><span className="client-icon"><Icon name="building"/></span><span><strong>{client.name}</strong><small>{isOpen?`${assigned} artículos activos asignados`:`${wh.length} bodegas · ${assigned} artículos activos`}</small></span><Status status={client.active?'Activo':'Inactivo'}/><Icon name="chevron"/></button>{isOpen&&<div className="client-detail"><div className="client-actions">{state.role==='Supervisor'&&<><button onClick={()=>form(client)}>Editar cliente</button><button onClick={()=>manage(client)}>Gestionar artículos</button></>}</div><h3>Bodegas</h3>{wh.map(w=><button className="warehouse-row" key={w.id} onClick={()=>state.role==='Supervisor'?warehouseForm(client.id,w):setModal(<Modal title={w.name} onClose={()=>setModal(null)}><p>{w.active?'Bodega activa':'Bodega inactiva'}</p><p>Último levantamiento: {fmtDate(w.lastCountAt)}</p><Button onClick={()=>setModal(null)}>Cerrar</Button></Modal>)}><Icon name="warehouse"/><span><strong>{w.name}</strong><small>{w.lastCountAt?`Último levantamiento: ${fmtDate(w.lastCountAt)}`:'Sin levantamientos'}</small></span><Status status={w.active?'Activa':'Inactiva'}/><Icon name="chevron"/></button>)}{state.role==='Supervisor'&&client.active&&<Button variant="secondary" onClick={()=>warehouseForm(client.id)}><Icon name="plus"/>Agregar bodega</Button>}</div>}</Card>})}</div></>; }
function ClientForm({ state,client,close,actions}:{state:AppState;client?:Client;close:()=>void;actions?:ServerActions}){const [name,setName]=useState(client?.name??'');const [active,setActive]=useState(client?.active??true);const [error,setError]=useState('');const [saving,setSaving]=useState(false);const save=async()=>{const normalized=name.trim();if(!normalized){setError('El nombre es obligatorio.');return}if(state.clients.some(c=>c.id!==client?.id&&c.name.toLowerCase()===normalized.toLowerCase())){setError('Ya existe un cliente con este nombre.');return}setError('');setSaving(true);const next={id:client?.id??id('client'),name:normalized,active};const ok=await actions?.saveClient(next,client?'Cliente actualizado':'Cliente creado');setSaving(false);if(ok)close();else setError('No se pudo guardar. Verifica permisos o conexión.');};return <Modal title={client?'Editar cliente':'Nuevo cliente'} onClose={close}><label>Nombre<input value={name} onChange={e=>setName(e.target.value)}/></label><label className="check-row"><input type="checkbox" checked={active} onChange={e=>setActive(e.target.checked)}/>Activo</label>{error&&<p className="field-error">{error}</p>}<Button onClick={save} disabled={saving}>{saving?'Guardando...':'Guardar cliente'}</Button><Button variant="secondary" onClick={close}>Cancelar</Button></Modal>}
function WarehouseForm({ state,clientId,warehouse,close,actions}:{state:AppState;clientId:string;warehouse?:Warehouse;close:()=>void;actions?:ServerActions}){const [name,setName]=useState(warehouse?.name??'');const [active,setActive]=useState(warehouse?.active??true);const [error,setError]=useState('');const [saving,setSaving]=useState(false);const save=async()=>{const normalized=name.trim();if(!normalized){setError('El nombre es obligatorio.');return}if(state.warehouses.some(w=>w.clientId===clientId&&w.id!==warehouse?.id&&w.name.toLowerCase()===normalized.toLowerCase())){setError('Ya existe una bodega con este nombre.');return}setError('');setSaving(true);const next={id:warehouse?.id??id('warehouse'),clientId,name:normalized,active,lastCountAt:warehouse?.lastCountAt};const ok=await actions?.saveWarehouse(next,warehouse?'Bodega actualizada':'Bodega agregada');setSaving(false);if(ok)close();else setError('No se pudo guardar. Verifica permisos o conexión.');};return <Modal title={warehouse?'Editar bodega':'Agregar bodega'} onClose={close}><label>Nombre<input value={name} onChange={e=>setName(e.target.value)}/></label><label className="check-row"><input type="checkbox" checked={active} onChange={e=>setActive(e.target.checked)}/>Activa</label>{error&&<p className="field-error">{error}</p>}<Button onClick={save} disabled={saving}>{saving?'Guardando...':'Guardar bodega'}</Button><Button variant="secondary" onClick={close}>Cancelar</Button></Modal>}
function Assignments({client,state,close,actions}:{client:Client;state:AppState;close:()=>void;actions?:ServerActions}){const [query,setQuery]=useState('');const initial=state.clientArticles.filter(a=>a.clientId===client.id&&a.active).map(a=>a.articleId);const [assigned,setAssigned]=useState<Set<string>>(()=>new Set(initial));const [error,setError]=useState('');const [saving,setSaving]=useState(false);const articles=state.articles.filter(a=>a.active&&a.name.toLowerCase().includes(query.toLowerCase()));const articleIds=state.articles.filter(a=>a.active).map(a=>a.id);const toggle=(articleId:string)=>setAssigned(current=>{const next=new Set(current);if(next.has(articleId))next.delete(articleId);else next.add(articleId);return next});const save=async()=>{setSaving(true);setError('');const ok=await actions?.saveAssignments(client.id,articleIds,[...assigned]);setSaving(false);if(ok)close();else setError('No se pudieron guardar las asignaciones. Verifica permisos o conexión.');};return <Modal title="Artículos asignados" onClose={close}><p>{client.name}</p><label className="search-box"><Icon name="search"/><input aria-label="Buscar artículo asignable" placeholder="Buscar artículo" value={query} onChange={e=>setQuery(e.target.value)}/></label><div className="assignment-list">{articles.slice(0,12).map(a=><label key={a.id} className="check-row"><input type="checkbox" checked={assigned.has(a.id)} onChange={()=>toggle(a.id)}/><span>{a.name}<small>{a.code}</small></span></label>)}</div>{error&&<p className="field-error">{error}</p>}<Button onClick={save} disabled={saving}>{saving?'Guardando...':'Guardar asignaciones'}</Button></Modal>}

function Profile({ state,go,update,setModal,signOut }: Pick<ScreenProps,'state'|'go'|'update'|'setModal'> & { signOut: () => void }){const user=state.users.find(u=>u.role===state.role)!;const reset=()=>setModal(<Modal title="Restablecer datos" onClose={()=>setModal(null)}><p>Esta acción solo restablece datos locales de pantallas fuera del flujo conectado.</p><Button variant="danger" onClick={()=>{update(()=>resetState(),'Datos locales restablecidos');setModal(null)}}>Restablecer datos locales</Button><Button variant="secondary" onClick={()=>setModal(null)}>Cancelar</Button></Modal>);const advance=(hours:number)=>update(s=>({...s,demoNow:new Date(new Date(s.demoNow).getTime()+hours*3600000).toISOString()}),`Reloj avanzado ${hours} h`);return <><Header title="Perfil"/><div className="screen-pad profile"><Card className="profile-card"><span className="profile-avatar">{user.avatar}</span><div><h2>{user.name}</h2><p>{state.role} · Supabase</p></div><span className="header-spacer"/></Card><Button variant="secondary" onClick={signOut}><Icon name="lock"/>Cerrar sesión</Button><Card><h2>Reloj de demostración</h2><strong className="demo-clock">{fmtDate(state.demoNow,true)}</strong><div className="clock-actions"><Button variant="secondary" onClick={()=>advance(7)}>Avanzar 7 h</Button><Button variant="secondary" onClick={()=>advance(19)}>Avanzar 19 h</Button><Button variant="plain" onClick={()=>update(s=>({...s,demoNow:new Date().toISOString()}),'Reloj restaurado')}>Restaurar reloj</Button></div></Card>{state.role==='Supervisor'&&<Button variant="secondary" onClick={()=>go('/clientes')}><Icon name="building"/>Clientes y bodegas</Button>}<Card><h2>Códigos de prueba</h2><p>Abre o imprime el PDF con los 15 códigos Code 128 usados por la cámara.</p><a className="button button--secondary button-link" href="/codigos_de_barras_code128.pdf" target="_blank" rel="noreferrer"><Icon name="scan"/>Abrir PDF Code 128</a></Card><Card><h2>Datos</h2><p>El flujo principal y los catálogos administrativos consultan y guardan datos en Supabase.</p><Button variant="danger" onClick={reset}><Icon name="refresh"/>Restablecer datos locales</Button></Card></div></>;}
function RoleModal({state,close,go}:{state:AppState;close:()=>void;go:(p:string)=>void}){return <Modal title="Rol de Supabase" onClose={close}><p>Tu rol actual es {state.role}. Los permisos administrativos dependen del perfil enlazado en Supabase.</p><Button onClick={()=>{close();go('/perfil')}}>Abrir Perfil</Button></Modal>}
