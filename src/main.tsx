...
import type { AppState, Article, Client, CountLine, InventoryCount, Lot, Role, UnitType, Warehouse } from './types';
import './styles.css';
...
function App() {
  ...
  const roleUser = state.users.find((u) => u.role === state.role)!;
  const router = () => {
    if (path === '/levantamientos' || path === '/') return <Dashboard state={state} roleUser={roleUser} go={go} setModal={setModal}/>;
    if (path === '/levantamientos/nuevo') return <NewCount state={state} go={go} back={back} update={update} setModal={setModal}/>;
    if (path.includes('/conteo')) return <CountView state={state} path={path} go={go} back={back} update={update} setModal={setModal}/>;
    if (path.includes('/lotes')) return <LotsViewEnhanced state={state} path={path} go={go} back={back} update={update} setModal={setModal}/>;
    if (path.includes('/revision')) return <ReviewView state={state} path={path} go={go} back={back} update={update} setModal={setModal}/>;
    ...
    if (path === '/perfil') return <Profile state={state} go={go} update={update} setModal={setModal}/>;
    return <Dashboard state={state} roleUser={roleUser} go={go} setModal={setModal}/>;
  };
  return <main className="page-shell"><div className="app-canvas">{router()}<BottomNav path={path} navigate={go}/>{toast && <Toast>{toast}</Toast>}{modal}</div></main>;
}

type ScreenProps = { state: AppState; path?: string; go: (path: string) => void; back?: () => void; update: (fn: (s: AppState) => AppState, message?: string) => void; setModal: (node: React.ReactNode) => void };
...
function LotsViewEnhanced({ state, path = '', go, back, update, setModal }: ScreenProps) {
  ...
  const save = () => {
    ...
    update((current) => ({ ...current, lines: [...current.lines.filter((item) => item.id !== next.id), next] }), 'Lotes guardados');
    go(`/levantamientos/${countId}/conteo?articulo=${articleId}`);
  };
...
function Profile({ state,go,update,setModal }: Pick<ScreenProps,'state'|'go'|'update'|'setModal'>){const user=state.users.find(u=>u.role===state.role)!;const choose=()=>setModal(<Modal title="Cambiar modo de demostración" onClose={()=>setModal(null)}><p>Los roles son simulados y solo modifican acciones visibles.</p>{(['Contador','Supervisor'] as Role[]).map(role=><Button key={role} variant={role===state.role?'primary':'secondary'} onClick={()=>{update(s=>({...s,role}),`Modo ${role} activado`);setModal(null)}}>{role}</Button>)}</Modal>);const reset=()=>setModal(<Modal title="Restablecer datos" onClose={()=>setModal(null)}><p>Se perderán los cambios locales de esta demostración.</p><Button variant="danger" onClick={()=>{update(()=>resetState(),'Datos de demostración restablecidos');setModal(null)}}>Restablecer datos</Button><Button variant="secondary" onClick={()=>setModal(null)}>Cancelar</Button></Modal>);const advance=(hours:number)=>update(s=>({...s,demoNow:new Date(new Date(s.demoNow).getTime()+hours*3600000).toISOString()}),`Reloj avanzado ${hours} h`);return <><Header title="Perfil"/><div className="screen-pad profile"><Card className="profile-card"><span className="profile-avatar">{user.avatar}</span><div><h2>{user.name}</h2><p>{state.role} · modo de demostración</p></div><button className="mini-edit" aria-label="Cambiar rol" onClick={choose}><Icon name="edit"/></button></Card><Card><h2>Reloj de demostración</h2><strong className="demo-clock">{fmtDate(state.demoNow,true)}</strong><div className="clock-actions"><Button variant="secondary" onClick={()=>advance(7)}>Avanzar 7 h</Button><Button variant="secondary" onClick={()=>advance(19)}>Avanzar 19 h</Button><Button variant="plain" onClick={()=>update(s=>({...s,demoNow:'2026-07-17T22:00:00-06:00'}),'Reloj restaurado')}>Restaurar reloj</Button></div></Card>{state.role==='Supervisor'&&<Button variant="secondary" onClick={()=>go('/clientes')}><Icon name="building"/>Clientes y bodegas</Button>}<Card><h2>Códigos de prueba</h2><p>Abre o imprime el PDF con los 15 códigos Code 128 usados por la cámara.</p><a className="button button--secondary button-link" href="/codigos_de_barras_code128.pdf" target="_blank" rel="noreferrer"><Icon name="scan"/>Abrir PDF Code 128</a></Card><Card><h2>Datos locales</h2><p>La información se guarda solo en este navegador para fines de demostración.</p><Button variant="danger" onClick={reset}><Icon name="refresh"/>Restablecer datos de demostración</Button></Card></div></>;}
